...
/*
    This UIApplication category ensures that OneSignal init() gets called at
   least one time

    If this did not occur, cold-start notifications would not trigger the
   React-Native 'opened' event and other miscellaneous problems would occur.

    First, we swizzle UIApplication's setDelegate method, to get notified when
   the app delegate is assigned. Then we swizzle UIApplication's
   didFinishLaunchingWithOptions() method. When this method gets called, it
   initializes the OneSignal SDK with a nil app ID.
*/

// helper method to swizzle instance methods
static void injectSelector(Class newClass, SEL newSel, Class addToClass,
                           SEL makeLikeSel) {
  ...
  dispatch_once(&onceToken, ^{
    method_exchangeImplementations(
        ...
        class_getInstanceMethod(self,
                                @selector(setOneSignalReactNativeDelegate:)));
  });
}
