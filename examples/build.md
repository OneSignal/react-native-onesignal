...
1. **Debug Console** (Collapsible log viewer at top - Appium-ready)
2. **App Section** (App ID, Guidance Banner, Consent Toggles, User Status Card, Login/Logout)
3. **Push Section** (Push ID, Enabled Toggle, Prompt Push)
4. **Notification Demo Section** (Simple, With Image, Custom buttons + notification templates)
5. **In-App Messaging Section** (Pause toggle)
6. **IAM Demo Section** (Top Banner, Bottom Banner, Center Modal, Full Screen - with icons)
...
11. **Outcome Section** (Send Outcome dialog with type selection)
...
15. **Live Activities Section** (iOS-only, setup/start/update/end)

### Prompt 2.1 - Debug Console (Top of Screen, Appium-Ready)
...
- Fixed height (100dp equivalent in React Native)
- Header bar with "Console" title, log count "(N)", and collapse/expand toggle
- Scrollable text area showing log messages (single vertical scroll)
- Trash icon button for clearing logs (not a text button)
- All SDK events and user actions are logged here via OSLog function
- Auto-scrolls to newest entries using ScrollView ref
...
Appium Semantic Labels (accessibilityLabel props):
| Label                  | Description                          |
|------------------------|--------------------------------------|
| log_view_container     | Main container View                  |
| log_view_header        | Tappable expand/collapse header      |
| log_view_count         | Shows "(N)" log count Text           |
| log_view_clear_button  | Trash icon TouchableOpacity          |
| log_view_list          | Scrollable content area              |
| log_view_empty         | "No logs yet" empty state Text       |
| log_entry_N            | Each log row (N=index)               |
| log_entry_N_timestamp  | Timestamp Text                       |
| log_entry_N_level      | D/I/W/E level indicator Text         |
| log_entry_N_message    | Log message content Text             |

Use accessibilityLabel prop for Appium:
<Text accessibilityLabel={`log_entry_${index}_message`}>{entry.message}</Text>
```

### Prompt 2.2 - App Section

```
App Section layout (combines App Info and User into one section):
...
- Notification permission is automatically requested when home screen loads:
  - Call OneSignal.Notifications.requestPermission() in useEffect on mount
...
### Prompt 2.4 - Notification Demo Section
...
### Prompt 2.6 - IAM Demo Section

```
IAM Demo Section (Send In-App Message):
- Section title: "Send In-App Message" with info icon for tooltip
- Four full-width buttons:
  1. TOP BANNER    - icon: align-top,    trigger: "iam_type" = "top_banner"
  2. BOTTOM BANNER - icon: align-bottom, trigger: "iam_type" = "bottom_banner"
  3. CENTER MODAL  - icon: crop-square,  trigger: "iam_type" = "center_modal"
  4. FULL SCREEN   - icon: fullscreen,   trigger: "iam_type" = "fullscreen"
- Button styling:
...
- On click: adds trigger "iam_type" with the type's value, shows Toast "Sent In-App Message: {type}"
...
### Prompt 2.15 - Live Activities Section (iOS Only)
...
### Prompt 2.16 - Details Screen (Secondary Tab)
...
- isLoading flag in AppStateContext
- Show/hide via isLoading state
- IMPORTANT: Add 100ms delay after populating data before dismissing loading indicator
  - This ensures UI has time to render before the spinner disappears
  - Use setTimeout(..., 100) after dispatching SET_LOADING(false)

On cold start (useEffect in OSDemo.tsx):
- Call OneSignal.initialize(APP_ID)
- Set debug log level
- Auto-prompt notification permission (call requestPermission here)
...
- If ID exists: show loading -> call fetchAndPopulateUserData() -> delay 100ms -> hide loading
- If null or error: wait for user change event to trigger fetch

On login (App Section):
...
- After populating: delay 100ms -> hide loading
...
- After dispatching all state: await delay(100) then dispatch SET_LOADING(false)
...
Fetch on app startup (non-blocking, fire-and-forget).
...
```
Create utils/TooltipHelper.ts as a singleton:

class TooltipHelper {
  private static _instance: TooltipHelper;
  private _tooltips: Record<string, TooltipData> = {};
  ...
  async init(): Promise<void> {
    ...
    try {
      // Fetch tooltip_content.json from remote URL using fetch()
      // Parse JSON into _tooltips map
      // On failure, leave _tooltips empty
    } catch (_) {}
...
- insetPadding equivalent for full-width dialogs
...
- Dividers between rows for visual clarity
- "Add Row" button to add more rows
- X (close) icon to delete a row (hidden when only one row)
- "Add All" button disabled until ALL key and value fields in every row are filled
...
- One checkbox per item, showing only the key as the label (not "key: value")
- "Remove (N)" button showing count of selected items, disabled when none selected
- On confirm, checked items' keys collected as string[] and passed to callback
...
- Event name input (required, shows error if empty on submit)
...
### Prompt 8.4 - Toast/SnackBar Messages

```
All user actions should display Toast messages (via ToastAndroid on Android,
Alert or a custom Toast component on iOS):

- Login: "Logged in as: {userId}"
- Logout: "Logged out"
- Add alias: "Alias added: {label}"
- Add multiple aliases: "{count} alias(es) added"
- Add email: "Email added: {email}"
- Remove email: "Email removed: {email}"
- Add SMS: "SMS added: {number}"
- Remove SMS: "SMS removed: {number}"
- Add tag: "Tag added: {key}"
- Add multiple tags: "{count} tag(s) added"
- Remove tag: "Tag removed: {key}"
- Remove selected tags: "{count} tag(s) removed"
- Add trigger: "Trigger added: {key}"
- Add multiple triggers: "{count} trigger(s) added"
- Remove trigger: "Trigger removed: {key}"
- Clear triggers: "All triggers cleared"
...
All Toast messages are also logged via OSLog().
```
...
│   └── sections/
│       ├── AppSection.tsx           # App ID, consent, user status, login/logout
│       ├── AliasesSection.tsx       # Alias management
│       ├── EmailSection.tsx         # Email subscriptions
│       ├── IamDemoSection.tsx       # Send IAM triggers (with icons)
...
├── utils/
│   └── TooltipHelper.ts             # Remote tooltip fetching + display utility
├── App.tsx                          # Root component with navigation
├── DetailsScreen.tsx                # Secondary tab (user/device info)
├── OSDemo.tsx                       # Main demo screen
├── OSConsole.tsx                    # Debug log console component (Appium-ready)
...
- Tooltip content is fetched from remote URL at runtime (not bundled locally)
- Debug console at top of screen displays SDK and app logs with Appium semantic labels
- Navigation uses bottom tabs with Home (OSDemo) and Details screens
- App Section combines App Info and User into a single section
...
- **No `as any`** - avoid type assertions; use proper TypeScript types and interfaces
- **Functional components** with hooks throughout; no class components
- **useCallback / useMemo** on handlers and derived values passed to child components to avoid unnecessary re-renders
- **Single responsibility** per file: one component per file, sections split into their own files
- **TextInput ref management** - clear and reset inputs after dialog submission
- **useFocusEffect** for SDK listener registration / cleanup tied to screen focus lifecycle
- **async/await** over raw Promise chaining for readability
- **try/catch** on all SDK async calls and network requests
- **AccessibilityLabel props** on interactive elements and log entries for Appium test automation
- **Consistent error handling** - errors logged via OSLog, user-facing feedback via Toast
- **Immutable state updates** in reducer - always return new objects/arrays, never mutate
- **No global mutable state** outside of React Context
...
- User management (login/logout, always-visible status card, aliases with batch add)
- Push notifications (subscription, sending with images, auto-permission prompt, notification templates)
...
1. **Testable** - Empty dialogs with Appium-ready accessibilityLabel props
...
6. **Responsive UI** - Loading overlay with 100ms delay to ensure UI populates before dismissing
7. **Debuggable** - Collapsible log console at top of screen with Appium semantic labels
8. **Modern UI** - OneSignal brand colors with consistent component library and icons on IAM buttons
...
11. **Remote tooltips** - Tooltip content fetched from sdk-shared repo at runtime, not bundled locally
